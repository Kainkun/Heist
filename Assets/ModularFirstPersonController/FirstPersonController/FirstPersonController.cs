...
public class FirstPersonController : MonoBehaviour
{
    ...
    private bool zoomButtonDown;
    private bool zoomButtonUp;
    ...
    private void Update()
    {
        ...
        if (enableZoom)
        {
            ...
            if(zoomButtonDown && !holdToZoom && !isSprinting)
            {
                if (!isZoomed)
                {
                    isZoomed = true;
        ...
        if (enableCrouch)
        {
            if(crouchButtonDown && !holdToCrouch)
            {
                Crouch();
            }
            ...
            else if(crouchButtonUp && holdToCrouch)
            {
                isCrouched = true;
                Crouch();
            }
    ...
    void FixedUpdate()
    {
        ...
        if (playerCanMove)
        {
            ...
            if (enableSprint && sprintButton && sprintRemaining > 0f && !isSprintCooldown)
            {
